-- ============================================================================
-- Migration: Referral Types (Phase 5)
-- ============================================================================
-- Uses sales.referral_type + commission_referral_types for rates.
-- Referrer paid only at phase 3 when referrer_phase_only = 3.
-- Ejecutivo/referrer rates override standard for referral sales.
-- ============================================================================

-- Optional: referrer recipient override (e.g. antonio_rada for portfolio_antonio_rada)
ALTER TABLE public.commission_referral_types
  ADD COLUMN IF NOT EXISTS referrer_recipient_id text,
  ADD COLUMN IF NOT EXISTS referrer_recipient_name text;

COMMENT ON COLUMN public.commission_referral_types.referrer_recipient_id IS
  'When set, pay this recipient instead of generic referral. E.g. antonio_rada for portfolio_antonio_rada.';

UPDATE public.commission_referral_types
SET referrer_recipient_id = 'antonio_rada', referrer_recipient_name = 'Antonio Rada'
WHERE id = 'portfolio_antonio_rada';

-- Update calculate_commissions with referral type logic
...
COMMENT ON FUNCTION public.calculate_commissions(uuid) IS
  'Calculates commissions. Project/period rates, escalation, referral types, 60% cap, ahorro por retiro.';
