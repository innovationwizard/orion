...
async function extractFromSheet(
  ...
  // Define row boundaries where desistimientos (cancellations) start
  // These sections should be excluded from the import
  const DESISTIMIENTO_BOUNDARIES: Record<string, number> = {
    'SANTA ELISA': 81,      // Desistimientos start after row 80 (0-indexed: 80)
    'BOULEVARD 5': 306,     // Desistimientos start after row 305 (0-indexed: 305)
    'BENESTARE ': 176,      // Desistimientos start after row 175 (0-indexed: 175)
    'BL-TAPIAS': 126,       // Desistimientos start after row 125 (0-indexed: 125)
  };
  
  const maxDataRow = DESISTIMIENTO_BOUNDARIES[sheetName] || 999999;
  
  // Keywords that indicate summary/aggregate sections (stop processing)
  const STOP_KEYWORDS = ['TOTAL', 'FLUJO', 'DESISTIMIENTO', 'DESESTIMIENTO', 'PORCENTAJE', 'DIFERENCIA', 'PPTO', 'ESTIMADO'];
  
  for (let rowIdx = headerRowIndex + 1; rowIdx < data.length; rowIdx++) {
    const row = data[rowIdx];
    if (!row || row.length === 0) continue;
    
    // Stop if we've reached the desistimientos section
    if (rowIdx >= maxDataRow) {
      console.log(`   ⏭️  Stopped at row ${rowIdx + 1} (desistimientos boundary - ${DESISTIMIENTO_BOUNDARIES[sheetName] ? 'excluded' : 'no boundary set'})`);
      break;
    }
    ...
    if (hasSummaryKeyword) {
      console.log(`   ⏭️  Stopped at row ${rowIdx + 1} (summary section keyword detected)`);
      break;
    }
    ...
    // Calculate deed_signed_date heuristic:
    // If all down payments are made (sum of payments >= down payment amount), use last payment date
    const totalPaid = payments.reduce((sum, p) => sum + p.amount, 0);
    const downPaymentComplete = totalPaid >= finalDownPayment * 0.95; // 95% threshold for rounding
    const deedDate = downPaymentComplete && payments.length > 0
      ? payments[payments.length - 1].date  // Last payment date
      : null;
    ...
    sales.push({
      ...
      promiseSignedDate: typeof reservationDateRaw === 'number' 
        ? excelDateToISO(reservationDateRaw)  // Use reservation date as promise signed date
        : null,
      deedSignedDate: deedDate,  // Set if down payment is complete
